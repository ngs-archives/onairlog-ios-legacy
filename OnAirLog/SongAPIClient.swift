      ...
      success: { (task: NSURLSessionDataTask!, responseObject: AnyObject!) -> Void in
        ...
        MagicalRecord.saveWithBlock({ (context: NSManagedObjectContext!) -> Void in
          for var i = 0; i < songs?.count; i++ {
            ...
            Song.findOrCreateWithAttributes(songDict as! [NSObject : AnyObject], inContext: context)
          }
          },
          completion: { (isSuccess: Bool, error: NSError?) -> Void in
            ...
            if error == nil {
              ...
              tracker.send(GAIDictionaryBuilder.createExceptionWithDescription(
                error!.localizedDescription, withFatal: false).build() as [NSObject : AnyObject])
              failure(task, error)
            }
        })
