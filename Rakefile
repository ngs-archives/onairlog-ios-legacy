...
namespace :env do
  ...
  task :export => :dotenv do
    ...
    file = File.join __dir__, APP_COMMON_NAME, 'Environment.swift'
...
class AdHocPage
  ...
  def plist_print(key)
    ...
    Shenzhen::PlistBuddy.print INFO_PLIST, key
  end
  def filesize
